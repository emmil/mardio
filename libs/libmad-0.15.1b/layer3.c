		 ...
		 unsigned char const sfbwidth[39])
{
  ...
  uc_memcpy(&xr[18 * sb], &tmp[sb], (576 - 18 * sb) * sizeof(mad_fixed_t));
}
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (stream->main_data == 0) {
    stream->main_data = lma_malloc(MAD_BUFFER_MDLEN);
    if (stream->main_data == 0) {
  ...
  if (frame->overlap == 0) {
    frame->overlap = lma_calloc(2 * 32 * 18, sizeof(mad_fixed_t));
    if (frame->overlap == 0) {
      stream->error = MAD_ERROR_NOMEM;
      return -1;
    }
  ...
  if (si.main_data_begin == 0) {
	...
	uc_memcpy(*stream->main_data + stream->md_len,
